...
def load(request):
    ...
    return render(request, 'vinos.html', {'vinos' : load_product(), 'quantity': carts.load_carts(request)})
...
def index(request):
    ...
    if request.method == 'GET': 
        tipos = SubTipoProducto.objects.filter(id_sub_tipo_producto__in=[5,6])
        products = Productos.objects.filter(id_tipo_producto='2')
        return render(request, "vinos.html", {"tipos": tipos, "products": products})
    else: 
